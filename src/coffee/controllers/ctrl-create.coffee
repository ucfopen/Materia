...
app.controller 'createCtrl', ($scope, $sce, $timeout, widgetSrv) ->
	...
	initCreator = ->
		dfd = $.Deferred().resolve()
		if inst_id?
			sendToCreator 'initExistingWidget', [instance.name, instance.widget, keepQSet.data, keepQSet.version, BASE_URL, MEDIA_UPLOAD_URL]
		else
			sendToCreator 'initNewWidget', [widget_info, BASE_URL, MEDIA_UPLOAD_URL]
		dfd.promise()
