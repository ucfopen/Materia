...
2. Update your `.env` file with the URL the ngrok process is hosting from: `BASE_URL`, `MEDIA_URL`, `MEDIA_UPLOAD_URL`, `WIDGET_URL`, `STATIC_CROSSDOMAIN`, and `TOOL_URL`. Also ensure `PLATFORM_DOMAIN` matches the domain of your Canvas instance. You will have to restart your python container after setting these values.
3. Create new LTI-related keys: `manage.py rotate_keys.`
4. Create a new developer LTI key in your Canvas instance. Select "Paste JSON" and copy the json content provided at https://your-ngrok-instance.ngrok-free.app/lticonfig. Note that will we have to revisit the configuration in a later step once we have a registration UUID.
