...
class Lti
{
	...
	public static function on_play_start_event($play_id)
	{
		// Attempt to get an lti token, if it exists. If so, this is a re-play
		// of a lti widget
		if ($lti_token = \Input::param('ltitoken'))
		{
			static::session_link_lti_token_to_play($lti_token, $play_id);
		}
	}
	 ...
	 */
	public static function is_lti_user_a_content_creator()
	{
		...
		$launch_roles = explode(',', \Input::param('roles'));
	...
	public static function get_launch_vars()
	{
		...
		static::$lti_vars = (object) [
			...
			'consumer'       => $consumer,
			'custom_inst_id' => trim(\Input::param('custom_widget_instance_id', false)), // Some tools will pass which inst_id they want
	...
	protected static function session_get_lti_data($play_id)
	{
		$token = static::session_get_lti_token_from_play($play_id);
	...
	protected static function session_save_lti_data($launch, $play_id)
	{
	...
	protected static function create_lti_association_if_needed($item_id, $launch)
	{
		...
		if ($assoc && $assoc->item_id == $item_id) return true;
		...
		return static::save_lti_resource_id_to_widget_association($item_id, $launch);
	}
	...
	protected static function get_or_create_user($launch, $search_field, $auth_driver, $creates_users = false)
	{
		...
		\Event::trigger('lti_get_or_create_user', $launch->username);
	...
	protected static function save_lti_resource_id_to_widget_association($inst_id, $launch)
	{
		$assoc = static::find_widget_from_resource_id($launch->resource_id);
	...
	protected static function session_link_lti_token_to_play($token, $play_id)
	{
		\Session::set("lti-{$play_id}", $token);
	}

	protected static function session_unlink_lti_token_to_play($play_id)
	{
		\Session::delete("lti-{$play_id}");
	}

	protected static function session_get_lti_token_from_play($play_id)
	{
		return \Session::get("lti-{$play_id}", false);
	}
	...
	protected static function update_user_roles(\Model_User $user, $launch, $auth)
	{
		...
		{
			$auth->update_role($user->id, static::is_lti_user_a_content_creator());
		}
	}
