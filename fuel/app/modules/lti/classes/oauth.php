...
class Oauth
{
	public static function validate_post()
	{
		...
		{
			\Mateira\Log::profile(['invalid-oauth-received', $e->getMessage(), \Uri::current(), print_r(\Input::post(), 1)], 'lti-error-dump');
		}
	...
	public static function send_body_hashed_post($endpoint, $body, $secret)
	{
		...
		try
		{
			...
			if ($file)
			{
				...
				if ($result == false)
				{
					\Mateira\Log::profile(['passback-failure', $body, $response], 'lti-error-dump');
				}
				return $result;
			}
		}
		...
		{
			\Mateira\Log::profile(['send-oath-post-failure', $e->getMessage(), $endpoint, print_r($params, true)], 'lti-error-dump');
		}
