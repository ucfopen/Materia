<?php
return [

	'paths' => [
		'gfonts' => [
			'path' => 'https://fonts.googleapis.com/',
			'css_dir' => '',
		],
		'static' => [
			'path' => Config::get('materia.urls.static'),
			'js_dir' => 'js/',
		]
	],

	'groups' => [

		'js' => [
			'modernizr' => [
				'files' => ['plugins/modernizr.js'],
				'enabled' => true,
				'attr' => ['data-src' => 'modernizr'],
			],
			'jquery' => [
				'files' => ['jquery-1.7.2.js'],
				'enabled' => true,
				'attr' => ['data-src' => 'jquery'],
			],
			'core' => [
				'files' => ['default.js',],
				'enabled' => true,
				'attr' => ['data-src' => 'core'],
			],
			'help' => [
				'files' => [
					'static::materia.flashcheck.js',
					'page/page.help.js',
					'swfobject.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'help'],
			],
			'homepage' => [
				'files' => [
					'notification.js',
					'static::materia.notification.js',
					'static::materia.coms.json.js',
					'static::materia.permissions.user.js',
					'static::materia.permissions.widget.js',
					'jquery-ui-1.8.21.custom.min.js',
					'static::materia.store.spotlight.js',
					'static::materia.store.slideshow.js',
				],
				'enabled' => false,
				'attr' => ['data-src' => 'homepage'],
			],
			'protected' => [
				'files' => [],
				'min' => false,
				'combine' => false,
			],
			'media_catalog' => [
				'files' => [
					'media-import.js',
					'jquery.dataTables.js',
					'static::materia.coms.json.js',
					'plugins/jquery.plupload.queue.js',
					'plugins/plupload.js',
					'plugins/plupload.flash.js',
					'plugins/plupload.html5.js',
					'plugins/plupload.html4.js',
				],
				'enabled' => false,
				'attr' => ['data-src' => 'media_catalog'],
			],
			'question_catalog' => [
				'files' => [
					'static::materia.questionimporter.js',
					'jquery.dataTables.js',
					'static::materia.coms.json.js',
				],
				'enabled' => false,
				'attr' => ['data-src' => 'question_catalog'],
			],
			'scores' => [
				'files' => [
					'static::materia.coms.json.js',
					'static::materia.scores.js',
					'static::materia.scores.scoregraphics.js',
					'plugins/excanvas.js',
					'plugins/jquery.jqplot.js',
					'plugins/jqplot_plugins/jqplot.barRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.categoryAxisRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.canvasTextRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.pointLabels.min.js',
					'plugins/jqplot_plugins/jqplot.canvasAxisLabelRenderer.min.js',
					'underscore-1.3.3.min.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'scores'],
			],
			'embed_scores' => [
				'files' => [
					'static::materia.coms.json.js',
					'static::materia.scores.js',
					'static::materia.scores.scoregraphics.js',
					'plugins/excanvas.js',
					'plugins/jquery.jqplot.js',
					'plugins/jqplot_plugins/jqplot.barRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.categoryAxisRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.canvasTextRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.pointLabels.min.js',
					'plugins/jqplot_plugins/jqplot.canvasAxisLabelRenderer.min.js',
					'underscore-1.3.3.min.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'embed_scores'],
			],
			'login' => [
				'files' => [
					'plugins/jquery.infieldlabel.js',
					'static::materia.set.availability.js',
					'static::materia.set.datetime.js',
					'login.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'login'],
			],
			'settings' => [
				'files' => [
					'page/page.settings.js',
					'plugins/spin.js',
					'plugins/spin.jquery.js',
					'static::materia.set.throbber.js',
				],
				'enabled' => false,
				'attr' => ['data-src' => 'settings'],
			],
			'profile' => [
				'files' => [
					'static::materia.profile.activity.load.js',
					'page/page.profile.js',
					'static::materia.coms.json.js',
					'plugins/date.js',
					'static::materia.set.datetime.js',
				],
				'enabled' => false,
				'attr' => ['data-src' => 'profile'],
			],
			'widget_catalog' => [
				'files' => [
					'catalog.js',
					'static::materia.notification.js',
					'static::materia.permissions.user.js',
					'static::materia.permissions.widget.js',
					'notification.js',
					'plugins/easing.js',
					'static::materia.coms.json.js',
					'plugins/jquery-css-transform.js',
					'plugins/jquery-animate-css-rotate-scale.js',
					'static::materia.widget.catalog.js',
					'static::materia.sorter.filter.js',
					'static::materia.set.throbber.js',
					'plugins/spin.js',
					'plugins/spin.jquery.js',
					'static::materia.image.js',
					'plugins/sorted.js',
				],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_catalog'],
			],
			'widget_detail' => [
				'files' => [
					'static::materia.image.js',
					'static::materia.coms.json.js',
					'static::materia.widget.detail.js',
					'plugins/jquery.fancybox-1.3.4.pack.js',
					'widget.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_detail'],
			],
			'widget_editor' => [
				'files' => [
					'static::materia.set.throbber.js',
					'plugins/jqmodal.js',
					'plugins/tablock.js',
					'jquery-ui-1.8.21.custom.min.js',
					'plugins/spin.js',
					'plugins/spin.jquery.js',
					'static::materia.coms.json.js',
					'swfobject.js',
					'static::materia.creator.js',
					'underscore-1.3.3.min.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_editor'],
			],
			'my_widgets' => [
				'files' => [
					'plugins/date.js',
					'static::materia.set.datetime.js',
					'static::materia.validate.textfield.js',
					'static::materia.notification.js',
					'static::materia.permissions.user.js',
					'static::materia.permissions.widget.js',
					'notification.js',
					'static::materia.image.js',
					'static::materia.textfilter.js',
					'plugins/jqmodal.js',
					'plugins/tablock.js',
					'static::materia.set.throbber.js',
					'plugins/spin.js',
					'plugins/spin.jquery.js',
					'static::materia.coms.json.js',
					'jquery-ui-1.8.21.custom.min.js',
					'plugins/jquery-ui-timepicker-addon.js',
					'static::materia.user.js',
					'static::materia.widget.js',
					'static::materia.widgetinstance.js',
					'static::materia.mywidgets.availability.js',
					'static::materia.mywidgets.selectedwidget.js',
					'static::materia.mywidgets.sidebar.js',
					'static::materia.mywidgets.statistics.js',
					'static::materia.mywidgets.tasks.js',
					'static::materia.mywidgets.csv.js',
					'static::materia.set.availability.js',
					'my_widgets.js',
					'plugins/excanvas.js',
					'plugins/jquery.jqplot.js',
					'plugins/jquery.qtip-1.0.0-rc3.min.js',
					'plugins/jqplot_plugins/jqplot.barRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.canvasTextRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.canvasAxisTickRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.categoryAxisRenderer.min.js',
					'plugins/jqplot_plugins/jqplot.cursor.min.js',
					'plugins/jqplot_plugins/jqplot.highlighter.min.js',
					'jquery.dataTables.js',
					'plugins/md5.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'my_widgets'],
			],
			'widget_play' => [
				'files' => [
					'plugins/jqmodal.js',
					'plugins/tablock.js',
					'jquery-ui-1.8.21.custom.min.js',
					'static::materia.coms.json.js',
					'swfobject.js',
					'static::materia.player.js'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_play'],
			],
		],

		'css' => [
			'widget_play' => [
				'files' => ['play.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_play'],
			],
			'my_widgets' => [
				'files' => [
					'my_widgets.css',
					'jquery.jqplot.min.css',
					'ui-lightness/jquery-ui-1.8.21.custom.css',
					'ui-lightness/jquery-ui-timepicker-addon.css',
					'jquery.dataTables.css'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'my_widgets'],
			],
			'widget_editor' => [
				'files' => ['create.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_editor'],
			],
			'widget_detail' => [
				'files' => ['jquery.fancybox-1.3.4.css', 'widget.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_catalog'],
			],
			'widget_catalog' => [
				'files' => ['catalog.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'widget_catalog'],
			],
			'profile' => [
				'files' => ['user.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'profile'],
			],
			'login' => [
				'files' => ['login.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'login'],
			],
			'scores' => [
				'files' => ['jquery.jqplot.min.css', 'scores.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'scores'],
			],
			'embed_scores' => [
				'files' => ['jquery.jqplot.min.css', 'scores_embedded.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'embed_scores'],
			],
			'question_catalog' => [
				'files' => ['jquery.dataTables.css','question-import.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'homepage'],
			],
			'media_catalog' => [
				'files' => [
					'jquery.dataTables.css',
					'jquery.plupload.queue.css',
					'media-import.css'
				],
				'enabled' => false,
				'attr' => ['data-src' => 'homepage'],
			],
			'homepage' => [
				'files' => ['store.css','widget.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'homepage'],
			],
			'help' => [
				'files' => ['docs.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'help'],
			],
			'404' => [
				'files' => ['404.css'],
				'enabled' => false,
				'attr' => ['data-src' => '404'],
			],
			'core' => [
				'files' => ['main.css'],
				'enabled' => true,
				'attr' => ['data-src' => 'core'],
			],
			'ie8' => [
				'files' => ['ie8.css'],
				'enabled' => false,
				'attr' => ['data-src' => 'ie8'],
			],
			'fonts' => [
				'files' => [
					'gfonts::css?family=Kameron:700&text=0123456789%25',
					'gfonts::css?family=Lato:300,400,700,700italic,900&amp;v2',
				],
				'attr' => ['data-src' => 'fonts'],
				'min' => false,
				'combine' => false,
			],
			'page' => [
				'files' => [],
				'attr' => ['data-src' => 'page'],
			],
		],
	],
];
