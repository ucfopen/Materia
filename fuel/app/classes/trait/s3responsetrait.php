...
trait Trait_S3ResponseTrait
{
	protected function add_s3_config_to_response()
	{
		...
		Js::push_inline('var MEDIA_URL = "'.($s3_enabled ? $s3_media_url : Config::get('materia.urls.media')).'";');
		Js::push_inline('var MEDIA_UPLOAD_URL = "'.($s3_enabled ? $s3_upload_url : Config::get('materia.urls.media_upload')).'";');
	}
}
