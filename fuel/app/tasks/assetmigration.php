...
class Assetmigration
{
	public static function run_asset_migration()
	{
		...
		$endpoint_url = (\Fuel::$env == \Fuel::DEVELOPMENT) ? 'aws --endpoint-url="'.$s3_config['upload_url'].':10001" ' : '';

		// S3 path should start with bucket, then prefix (subdir), then asset filename
		$upload_bucket = ($s3_config['uploads_bucket']) ? $s3_config['uploads_bucket'].'/' : '';

		// append a slash to subdirectory path if the config is using a subdirectory
		$sub_directory = ($s3_config['subdir']) ? $s3_config['subdir'].'/' : '';
		...
		foreach ($all_assets as $asset)
		{
			...
			$bucket_path = '{$upload_bucket}{$sub_directory}{$user_id}-{$asset_id}.{$asset_type}';

			fwrite($temp_bash_stream, $endpoint_url.'s3 cp '.$local_path.' s3://'.$bucket_path.'\n');
	...
	private static function accept_instructions($bash_filename, $sql_filename)
	{
		...
		return (\Cli::prompt('Continue with the migration? Y/n', ['Y', 'n']) == 'Y');
	}
	...
	public static function initialize_output_files($bash_filename, $sql_filename)
	{
		...
		$file_contents = '# This file holds all of the bash commands to upload Materia assets to Amazon s3.\n';
	...
	private static function generate_update_query($id, $remote_url)
	{
		...
		return 'UPDATE '.$asset_table_name.' SET remote_url="'.$remote_url.'", status = "migrated_asset" WHERE id="'.$id.'";\n';
	}
}
