...
H5PEditor.widgets.video = H5PEditor.widgets.audio = H5PEditor.AV = (function(
  ...
  function C(parent, field, params, setValue) {
    ...
    self.on("upload", function() {
      // Insert throbber
      self.$uploading = $(
        '<div class="h5peditor-uploading h5p-throbber">' +
  ...
  C.prototype.appendTo = function($wrapper) {
    ...
    if (!this.field.disableCopyright) {
      imageHtml +=
        '<a class="h5p-copyright-button" href="#">' +
    ...
    const toggleTab = function() {
      ...
      el.parentElement
        .querySelector(".av-tabpanel:not([hidden])")
        .setAttribute("hidden", "");
    ...
    if (self.field.widgetExtensions) {
      ...
      const createTabInstance = function(type, index) {
        ...
        tabInstance.appendTo(
          self.$addDialog[0].children[0].children[index + 1]
        ); // Compensate for .av-tablist in the same wrapper
  ...
  C.prototype.addFile = function(index) {
    ...
    var defaultQualityName = H5PEditor.t("core", "videoQualityDefaultLabel", {
      ":index": index + 1
    });
    var qualityName =
      file.metadata && file.metadata.qualityName
    ...
    if (this.field.enableCustomQualityLabel === true && !isYoutube) {
      fileHtml =
        ...
        '<div class="h5p-type" title="' +
        file.mime +
        '">' +
        file.mime.split("/")[1] +
        "</div>" +
        '<div role="button" tabindex="0" class="h5p-remove" title="' +
        H5PEditor.t("core", "removeFile") +
        '">' +
        "</div>" +
        "</div>" +
        ...
        "</div>" +
        '<label class="h5peditor-field-description" for="' +
        rowInputId +
        '">' +
        H5PEditor.t("core", "videoQualityDescription") +
        "</label>" +
        '<input id="' +
        rowInputId +
        '" class="h5peditor-text" type="text" maxlength="60" value="' +
        qualityName +
        '">' +
        "</div>" +
        "</li>";
    ...
    $file.children(".h5p-thumbnail").click(function() {
      ...
      that.$addDialog
        .addClass("h5p-open")
        .find(".h5p-file-url")
        .val(that.params[index].path);
      that.updateIndex = index;
    });

    // Handle remove button click
    $file.find(".h5p-remove").click(function() {
      if (that.$add.is(":visible")) {
  ...
  C.prototype.useUrl = function(url) {
    ...
    var index =
      this.updateIndex !== undefined ? this.updateIndex : this.params.length;
  ...
  C.createInsertDialog = function(content, disableInsert, id, hasDescription) {
    return (
      ...
      '"' +
      (hasDescription
        ? ' aria-describedby="' + ns.getDescriptionId(id) + '"'
        : "") +
      ' class="h5p-add-file" title="' +
      H5PEditor.t("core", "addFile") +
      '"></div>' +
      ...
      '<button class="h5peditor-button-textual h5p-insert"' +
      (disableInsert ? " disabled" : "") +
      ">" +
      H5PEditor.t("core", "insert") +
      "</button>" +
      '<button class="h5peditor-button-textual h5p-cancel">' +
    ...
    switch (tab) {
      case "BasicFileUpload":
        ...
        return (
          ...
          H5PEditor.t(
            "core",
            type === "audio" ? "uploadAudioTitle" : "uploadVideoTitle"
          ) +
          "</h3>" +
      ...
      case "InputLinkURL":
        return (
          ...
          H5PEditor.t(
            "core",
            type === "audio" ? "enterAudioTitle" : "enterVideoTitle"
          ) +
          "</h3>" +
          ...
          H5PEditor.t(
            "core",
            type === "audio" ? "enterAudioUrl" : "enterVideoUrl"
          ) +
          '" class="h5p-file-url h5peditor-text"/>' +
          "</div>" +
          (type === "audio"
            ? ""
            : '<div class="h5p-errors"></div><div class="h5peditor-field-description">' +
  ...
  C.createTabbedAdd = function(type, extraTabs, id, hasDescription) {
    ...
    const tabs = ["BasicFileUpload", "InputLinkURL"];
    ...
    for (i = 0; i < tabs.length; i++) {
      ...
      const title =
        i > 1
          ? H5PEditor.t("H5PEditor." + tab, "title")
          : H5PEditor.t("core", "tabTitle" + tab);
      ...
      tabsHTML +=
        ...
        "</div>";
      tabpanelsHTML +=
        '<div class="av-tabpanel" tabindex="-1" role="tabpanel" id="av-tabpanel-' +
        tabId +
        ...
        (i === 0 ? "" : ' hidden=""') +
        ">" +
        C.createTabContent(tab, type) +
        "</div>";
    }

    return C.createInsertDialog(
      '<div class="av-tablist" role="tablist" aria-label="' +
        H5PEditor.t("core", "avTablistLabel") +
        '">' +
        tabsHTML +
        "</div>" +
        tabpanelsHTML,
  ...
  C.createAdd = function(type, id, hasDescription) {
    return C.createInsertDialog(
      ...
      //   '</div>' +
      // '</div>' +
      '<div class="h5p-dialog-box" style="width:90%">' +
