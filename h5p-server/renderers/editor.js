			...
			H5PEditor.init = function() {
				...
				if (h5peditor === undefined) {
					...
					if (editTypeMatch == 'edit') {
						window.parent.postMessage({ message: 'ready_for_qset' }, "http://localhost:8118") // TODO add this url to a config somewhere?
					}
					// no contentId passed in, initialize empty editor
					else if (editTypeMatch == 'new') {
						h5peditor = new ns.Editor(window.selectedLibrary, undefined, $editor[0]);
						$create.show()
					}
					else {
